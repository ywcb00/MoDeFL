...
from tffmodel.ModelBuilderUtils import getOptimizer
...
class Initiator:
        ...
        async with grpc.aio.insecure_channel(addr) as channel:
            ...
            await stub.InitDataset(Initialization_pb2.Dataset(
                ...
                partition=Initialization_pb2.Partition(
                    ...
                    partition_dirichlet_alpha=self.config["partitioning_alpha"])))

            await stub.InitModel(Initialization_pb2.Model(
                modeltype_id=self.config["modeltype_id"].value,
            ...
            await stub.InitStrategy(Initialization_pb2.Strategy(
                ...
                sync_strat_spec=Initialization_pb2.SynchronizationStrategySpec(
                    strategy_id=self.config["sync_strategy"].value,
                    percentage=self.config["sync_strat_percentage"],
                    amount=self.config["sync_strat_amount"],
                    timeout=self.config["sync_strat_timeout"],
                    allow_empty=self.config["sync_strat_allowempty"]),
                ...
                pdp_strat_spec=Initialization_pb2.PartialDeviceParticipationStrategySpec(
                    strategy_id=self.config["pdp_strategy"].value,
                    k=self.config["pdp_k"])
                ))
    ...
    async def initialize(self, addresses, adj_mat):
        model = ModelUtils.getModelObject(self.config)
        model.initModelElementSpec(getDatasetElementSpec(self.config))
        model_config_serialized, optimizer_config_serialized = model.serialize()

        init_weights = model.getWeights()
